...
public class LAAandWiFiSimulator {
    ...
    1: interval_time -> Frequency allocation interval (execution interval of the proposed method)
    2: wifi_user_lambda -> WiFi only user arrival rate
    3: lteu_user_lambda -> Arrival rate of WiFi + LTE-U users
    4: end_condition -> Selection of simulation end conditions(0: number of calls, 1: time)
    5: end_num -> Number of calls or time that is the end condition
    6: service_set -> User's service usage pattern (0: File download, 1: Communication for a certain period of time)
...
//            scenario = new Scenario((40+_param.loop_num), _param, Topology);
            ...
            //The one that recorded the throughput of all users during the simulation → Used to get the cumulative distribution
//            if (_param.loop_num == 1) {
//                output.writeToFile_Throughput(scenario.getData().wifi_users_throghput, scenario.getData().lteu_users_throghput,
//                         scenario.getData().wifi_users_min_throghput, scenario.getData().lteu_users_min_throghput);
//
