...
/**
 * 提案手法6: 交叉:一様交叉 / 評価値:fairness index / 突然変異:条件⇒確率x%, 方法⇒Swap
 * 初期個体:前回のGAの個体を受け継ぐ(←EventOfGenericAlgorithm3.javaとの差分)
 *
 * @author ginnan
 */
public class EventOfGenericAlgorithm9 extends Event {
...
//    private static final int INDIVIDUAL_NUM = Constants.GA_INDIVIDUAL_NUM;
    ...
    public EventOfGenericAlgorithm9(double time, Scenario scenario) {
        super(scenario);
    ...
    private void StartAlgorithm3() throws IOException {//System.out.println("****");
//int deb =0;
        //デバッグ*****************
//        double th_p = 999999;
...
//          temp_individual = CreateFirstIndividualsChannelOn();
        ...
        //*****デバッグstart
//        int[][] temp_ind;
//        for(int d = 0; d < individuals.get(0).length; d++){
//            temp_ind = individuals.get(0);
//            System.out.print(" " + temp_ind[d][1]);
//        }System.out.println();
        //*******デバッグend
//System.out.println(_scenario.getIndividuals().size() +"\t" + individuals.size());
        /*初期集団の評価 */
...
//            if(x== GA_num - 1){System.out.println(individuals_evaluations.get(0) +"\t"+ "evalu");}
            /* 淘汰 */
            Select();
...
//        int[][] temp_ind2;
//        System.out.println("---------------------------------");
//        for(int d = 0; d < _scenario.getIndividuals().get(0).length; d++){
//            temp_ind2 = _scenario.getIndividuals().get(0);
//            System.out.print(" " + temp_ind2[d][1]);
...
//        System.out.println("Simulation Time: " + (end - start) + "[ms]");
        ...
        Event next_event = new EventOfGenericAlgorithm9(time, _scenario);
...
//          ReConnectNotChange(individual);
        ...
        for (int i = 0; i < Constants.LTEU_NUM; i++) {
            ...
            if (_lteu_bs[i].connecting_num != 0) {
                ...
                throughput_2jo += (temp_evaluation[i] * temp_evaluation[i]) * _lteu_bs[i].connecting_num;
        ...
        for (int i = 0; i < Constants.WiFi_NUM; i++) {
            ...
            if (_wifi_ap[i].connecting_num != 0) {
                ...
                throughput_2jo += (temp_evaluation[i + Constants.LTEU_NUM] * temp_evaluation[i + Constants.LTEU_NUM]) * _wifi_ap[i].connecting_num;
                total_connecting_num += _wifi_ap[i].connecting_num;
            }
        }

        //3. 評価値を決定:fairness index 
//        average_throughput = average_throughput / total_connecting_num;
        fair = (average_throughput * average_throughput) / (total_connecting_num * throughput_2jo);
//        if(temp_min_evaluataion == 3.5){
