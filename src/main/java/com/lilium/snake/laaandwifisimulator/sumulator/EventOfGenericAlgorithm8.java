...
/**
 * 提案手法8: EventOfGenericAlgorithm6.javaの高速化
 *
 * @author ginnan
 */
public class EventOfGenericAlgorithm8 extends Event {
...
//        LinkedList<UserNode>[] area_lteu_users = new LinkedList[Constants.AREA_NUM];
    public EventOfGenericAlgorithm8(double time, Scenario scenario) {
        super(scenario);
...
//        ReConnectNotChange(max_evaluate_individual);
        ...
        Event next_event = new EventOfGenericAlgorithm8(time, _scenario);
...
//        }
    ...
    private void Crossover() {
        ...
        for (int j = 0; j < Constants.PARENT_NUM; j++) {
            ...
            //2.交叉方法を選択して交叉
//             DoUniformCrossover(j); //20180122に変更
        }
...
//        individuals_evaluations.add(Evaluate(childrens[j].Parent2));
    ...
    private void DoUniformCrossover(int j) {
        ...
        for (int i = 0; i < temp_parents.Parent1.length; i++) {
            ...
            System.arraycopy(temp_parents.Parent2[i], 0, temp_individual_2[i], 0, temp_parents.Parent2[i].length);//20180122に修正
        }
...
//        }
        ...
        childrens[j].Parent2 = temp_individual_2;
        individuals.add(childrens[j].Parent1);
        individuals_evaluations.add(Evaluate2(childrens[j].Parent1));
//        evaluations_min.add(Evaluate_min());
        individuals.add(childrens[j].Parent2);
        individuals_evaluations.add(Evaluate2(childrens[j].Parent2));
//        evaluations_min.add(Evaluate_min());
    ...
    private void MutationSwap() {
        ...
        mutate_flag = select_individual;
        //突然変異した個体をもとの個体にコピー
        individuals.set(select_individual, mutate_individual);
        individuals_evaluations.set(select_individual, Evaluate2(mutate_individual));
//         evaluations_min.set(select_individual, Evaluate_min());
...
//            evaluations_min.remove(elite_index);
        ...
        for (int j = 0; j < individuals_num - Constants.ELITE_SELECT_NUM; j++) {
            do {
...
//        }
    ...
    private double Evaluate_min() {
        ...
        if (mutate_flag == 500) {
            evaluations_ids.set(mutate_flag, temp_id);
            mutate_flag = -1;
        } else {
            evaluations_ids.add(temp_id);
        }
    ...
    private void ProposedReConnect(int[][] individual) {
        ...
        for (int i = 0; i < Constants.WiFi_NUM; i++) {
            ...
            //AP,BSのユーザ情報のクリア ＆個体のチャネル割り当てに更新 20180122修正
//            _wifi_ap[i].assigned_channel = individual[i + Constants.LTEU_NUM][1];
