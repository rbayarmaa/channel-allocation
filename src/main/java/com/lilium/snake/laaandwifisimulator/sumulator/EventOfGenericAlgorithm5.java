...
/**
 * 提案手法5 : 交叉:一様交叉 / 評価値:最小スループット/ 突然変異：条件⇒確率x% or 最小TPのAPが全個体同一,方法⇒最小AP周り再割当てor
 * Swap 初期個体：前回のGAの個体を受け継ぐ (←EventOfGenericAlgorithm4.javaとの差分)
 *
 * @author ginnan
 */
public class EventOfGenericAlgorithm5 extends Event {
...
//    private static final int INDIVIDUAL_NUM = Constants.GA_INDIVIDUAL_NUM;
    ...
    public EventOfGenericAlgorithm5(double time, Scenario scenario) {
        super(scenario);
...
//            individuals.add(CreateFirstIndividuals());
        }
...
//System.out.println(unum);
        ...
        Event next_event = new EventOfGenericAlgorithm5(time, _scenario);
...
//System.out.println(target_id);
    ...
    private void Select() {
        ...
        individuals.clear();
        individuals_evaluations.clear();
        evaluations_ids.clear(); //20180109 add
...
//        individuals = next_individuals;
        while (!next_individuals.isEmpty()) {
            ...
            //20180109 add end
        }//System.out.println(individuals.size() + "\t" + individuals_evaluations.size());
    }
