...
/**
 * 提案手法7: 交叉:一様交叉 / 評価値:ユーザスループットの対数の平均 / 突然変異:条件⇒確率x%, 方法⇒Swap or 最小AP周り再割当て
 * 初期個体:前回のGAの個体を受け継ぐ(←EventOfGenericAlgorithm3.javaとの差分)
 *
 * @author ginnan
 */
public class EventOfGenericAlgorithm7 extends Event {
...
//    private static final int INDIVIDUAL_NUM = Constants.GA_INDIVIDUAL_NUM;
    ...
    public EventOfGenericAlgorithm7(double time, Scenario scenario) {
        super(scenario);
...
//System.out.println("++");
        ...
        Event next_event = new EventOfGenericAlgorithm7(time, _scenario);
...
//          ReConnectNotChange(individual);
        ...
        for (int i = 0; i < Constants.LTEU_NUM; i++) {
            ...
            if (_lteu_bs[i].connecting_num != 0) {
                ...
                temp_evaluation[i] = Utility.CalcUserthroughput(_lteu_bs[i].capacity, _lteu_bs[i].connecting_num);
                utility += Math.log(temp_evaluation[i] + 1) * _lteu_bs[i].connecting_num;
        ...
        for (int i = 0; i < Constants.WiFi_NUM; i++) {
            ...
            if (_wifi_ap[i].connecting_num != 0) {
                ...
                temp_evaluation[i + Constants.LTEU_NUM] = Utility.CalcUserthroughput(_wifi_ap[i].capacity, _wifi_ap[i].connecting_num);
                utility += Math.log(temp_evaluation[i + Constants.LTEU_NUM] + 1) * _wifi_ap[i].connecting_num;
                total_connecting_num += _wifi_ap[i].connecting_num;
            }
        }

        //3. 評価値を決定:スループットの対数の平均 
        utility = utility / total_connecting_num;
