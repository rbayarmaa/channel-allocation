...
//        writeToFile_avg_thr();
        ...
        //Event next_event = new EventOfGenericAlgorithm6(time, _scenario); //channel assignment-g neg udaa hiihiin tuld comment bolgoson
...
//        }//*****************************
        ...
        int bs_num = 1;
        while (bs_num != Constants.LTEU_AREA_COVER_NUM + 1) {//Constants.LTEU_AREA_COVER_NUMはLTE BSのカバーエリア同士が最大どのくらいカバーしているか→手動で変更
            for (int i = 0; i < Constants.AREA_NUM; i++) {
...
//        }
    ...
    public void writeToFile_avg_thr() throws IOException {
        FileWriter fw = new FileWriter("./WiFiLTEU_Data/AvgPerTime/" + _param.lteu_user_lambda + "AvgPerExecute.dat", true);
        BufferedWriter bw = new BufferedWriter(fw);
